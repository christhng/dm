...
<div class="w3-main" style="margin-left:340px;margin-right:40px">
  ...
  <div class="w3-container" style="margin-top:80px" id="svcinfo">
    ...
    <h1 class="w3-xxxlarge w3-text-red"><b>Search by Stop</b></h1>
...
if (!$conn) {
	exit( 'Could not connect:'              
	 ...
	 $searchStops = "%{$_POST['searchStops']}%";
	 $searchStopsdata =$_POST['searchStops'];
	
	 $pQuery = "select bs.stopnumber, address, count(distinct(servicenumber)) from bus_stop bs, bus_route br where br.stopnumber=bs.stopnumber and br.stopnumber like ? group by br.stopnumber;"; 
	 
	$stmt = mysqli_prepare($conn, $pQuery);
	
	mysqli_stmt_bind_param($stmt, 's', $searchStops);

	mysqli_stmt_execute($stmt);
	mysqli_stmt_bind_result($stmt, $stopNum, $stopAddr, $stopCountServed);
	//mysqli_stmt_fetch($stmt);

	print "<p>";
	print "Your current search is based on Stop Number equal or containing: ";
	print "$searchStopsdata";
	print "</p>";
	print "<table border='0'>";
	
	print "<tr><th>Stop Number</th><th>Stop Address</th><th>Service Numbers Served</th></tr>";

	while (mysqli_stmt_fetch($stmt)) {
		
	print "<tr><td>".$stopNum."</td><td>".$stopAddr."</td><td>".$stopCountServed."</td></tr>";
